...
const fs_1$4 = require$$2$2;
const isEqual = lodash_isequalExports;
const fs_extra_1$6 = fs;
const path$8 = path$9;
...
const fs_extra_1$5 = fs;
const fs_1$2 = require$$2$2;
...
class AppUpdater extends events_1.EventEmitter {
  ...
  async getOrCreateStagingUserId() {
    ...
    try {
      const id2 = await (0, fs_extra_1$4.readFile)(file, "utf-8");
...
function registerScanHandlers(mainWindow2) {
  require$$1$2.ipcMain.handle("scan:start", async (_, request) => {
...
function registerBulkScanHandlers(mainWindow2) {
  require$$1$2.ipcMain.handle("bulk:start", async (_, request) => {
...
function registerFileHandlers() {
  ...
  require$$1$2.ipcMain.handle("file:open-report", async (_, reportPath) => {
    await require$$1$2.shell.openPath(reportPath);
  });
  require$$1$2.ipcMain.handle("file:open-folder", async (_, folderPath) => {
...
Report: ${htmlPath}`
    ...
    await require$$1$2.shell.openExternal(`mailto:?subject=${subject}&body=${body}`);
  });
}
function registerAppHandlers() {
  require$$1$2.ipcMain.handle("app:version", () => {
    return require$$1$2.app.getVersion();
  });
  require$$1$2.ipcMain.handle("app:platform", () => {
    return process.platform;
  });
  require$$1$2.ipcMain.handle("app:reports-path", async () => {
...
function registerLicenseHandlers() {
  require$$1$2.ipcMain.handle("license:activate", async (_, key) => {
    const { activateLicense } = await Promise.resolve().then(() => require("./chunks/licenseValidator-CNuoQ2l9.js"));
    return activateLicense(key);
  });
  require$$1$2.ipcMain.handle("license:check", async () => {
    if (process.env.NODE_ENV === "development") {
      return { valid: true, license: null };
    }
    const { checkLicense } = await Promise.resolve().then(() => require("./chunks/licenseValidator-CNuoQ2l9.js"));
    return checkLicense();
  });
  require$$1$2.ipcMain.handle("license:deactivate", async () => {
    const { deactivateLicense } = await Promise.resolve().then(() => require("./chunks/licenseValidator-CNuoQ2l9.js"));
...
function initReportsDir(userDataPath) {
  _reportsDir = path$9.join(userDataPath, "reports");
}
...
function getScanArtifactsDir(scanId) {
  return path$9.join(getReportsDir(), scanId);
}
function getScreenshotsDir(scanId) {
  return path$9.join(getScanArtifactsDir(scanId), "screenshots");
}
function buildJsonPath(scanId) {
  return path$9.join(getScanArtifactsDir(scanId), "report.json");
}
...
function getBulkScansDir() {
  return path$9.join(getReportsDir(), "bulk");
}
function getBulkScanPath(batchId) {
  return path$9.join(getBulkScansDir(), `${batchId}.json`);
}
function getDiscoveryDir() {
  return path$9.join(getReportsDir(), "discovery");
}
function getDiscoveryPath(discoveryId) {
  return path$9.join(getDiscoveryDir(), `${discoveryId}.json`);
}
function getMarketDashboardsDir() {
  return path$9.join(getReportsDir(), "market-dashboards");
}
function getMarketDashboardPath(dashboardId) {
  return path$9.join(getMarketDashboardsDir(), `${dashboardId}.json`);
}
...
function initMonitoringDir(userDataPath) {
  _monitoringDir = path$9.join(userDataPath, "monitoring");
}
...
function getSitesPath() {
  return path$9.join(getMonitoringDir(), "sites.json");
}
function getSiteHistoryDir(siteId) {
  return path$9.join(getMonitoringDir(), "history", siteId);
}
function getScanSummaryPath(siteId, scanId) {
  return path$9.join(getSiteHistoryDir(siteId), `${scanId}.json`);
}
...
function getLicensePath() {
  ...
  return path$9.join(_userDataPath$1, "license.json");
}
...
function createWindow() {
  const win = new require$$1$2.BrowserWindow({
    ...
    webPreferences: {
      preload: path$9.join(__dirname, "../preload/index.js"),
  ...
  if (isDev && process.env["ELECTRON_RENDERER_URL"]) {
    ...
    win.loadFile(path$9.join(__dirname, "../renderer/index.html"));
  }
  return win;
}
require$$1$2.app.whenReady().then(() => {
  const userData = require$$1$2.app.getPath("userData");
  ...
  if (!isDev) {
    ...
    setTimeout(() => main$1.autoUpdater.checkForUpdates().catch(() => {
    }), 1e4);
  }
  require$$1$2.ipcMain.handle("update:install", () => {
