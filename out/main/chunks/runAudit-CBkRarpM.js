...
function generateHtml(r, branding = {}) {
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Agency branding */
    .agency-bar { display: flex; align-items: center; gap: 10px; margin-bottom: 20px; padding-bottom: 16px; border-bottom: 1px solid rgba(255,255,255,0.12); }
    .agency-logo { height: 32px; width: auto; object-fit: contain; border-radius: 4px; }
    .agency-name { font-size: 13px; font-weight: 600; color: #cbd5e1; letter-spacing: 0.3px; }
...
<div class="container">
  ...
  <div class="report-header">
    ${branding.agencyName || branding.agencyLogoBase64 ? `
    <div class="agency-bar">
      ${branding.agencyLogoBase64 ? `<img src="${branding.agencyLogoBase64}" alt="${escHtml(branding.agencyName ?? "")}" class="agency-logo" />` : ""}
      ${branding.agencyName ? `<span class="agency-name">${escHtml(branding.agencyName)}</span>` : ""}
    </div>` : ""}
    <h1>${branding.agencyName ? "SEO Audit Report" : "Local SEO Audit Report"}</h1>
  ...
  <div class="footer">
    ${branding.agencyName ? `Prepared by ${escHtml(branding.agencyName)}` : "Generated by Local SEO Scanner"} &nbsp;·&nbsp; ${date} &nbsp;·&nbsp; Scan ID: ${escHtml(r.id)}
  </div>
...
function estimateRevenueImpact(input) {
  ...
  const currencyMultiplier = input.currencyMultiplier ?? 1;
...
async function revenueStage(ctx, emit) {
  ...
  const settings = await settingsStorage.readSettings();
  const currencyConfig = settingsStorage.CURRENCY_CONFIG[settings.currency];
